    ...
    private func updateVideoInfo() {
        ...
        Task {
            if let updatedVideo = try? await networkService.request(router, responseType: PostResponseDTO.self).asShortVideoItem {
                await MainActor.run {
                    // shortVideo 객체를 새 객체로 갈아끼울 때, 원래의 prefetcher를 이용해서 delegate의 prefetcher를 다시 세팅해 주어야 한다.
                    // 만에 하나 없는 경우에는 메모리를 사용하도록 구현
                    // 이러한 구조는 개선이 필요하긴 할 듯..
                    let originalPrefetcher = self.shortVideo.resourceLoaderDelegate.prefetcher ?? ShortVideoMemoryPrefetcher.shared
